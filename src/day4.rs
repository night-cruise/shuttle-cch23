...
pub fn cursed_candy_eating_contest(reindeers: Json<Vec<Reindeer>>) -> Json<ReindeerSummary> {
    ...
    let fastest = findest(&|acc, i| {
        if reindeers[acc].speed < reindeers[i].speed {
            i
        } else {
            acc
        }
    ...
    let tallest = findest(&|acc, i| {
        if reindeers[acc].height < reindeers[i].height {
            i
        } else {
            acc
        }
    ...
    let magician = findest(&|acc, i| {
        if reindeers[acc].snow_magic_power < reindeers[i].snow_magic_power {
            i
        } else {
            acc
        }
    ...
    let consumer = findest(&|acc, i| {
        if reindeers[acc].c_an_d13s_3_a_te_n_yes_t3rd_ay
            < reindeers[i].c_an_d13s_3_a_te_n_yes_t3rd_ay
        {
            i
        } else {
            acc
